...
template<>
void mblock<scomp>::get_svals_LrM(float* sigma) const
{
  ...
  scomp* const tmp1 = new scomp[size];
  assert(tmp1!=NULL);

  // copy data to tmp1
  blas::copy(k*(n1+n2), data, tmp1);

  scomp* const tmp2 = tmp1+k*n1;  // tmp2 points to V

  // QR-Zerlegung von U und V
  int INFO;
  scomp* const WORK = tmp2 + n2*k;    // LWORK
  scomp* const tau1 = WORK + LWORK;   // k
  scomp* const tau2 = tau1 + k;       // k
...
template<>
void mblock<scomp>::convLrM_toGeM(unsigned k, scomp* U, unsigned ldU,
				  scomp* V, unsigned ldV)
{
  if (k>0) {
    setGeM();
    blas::gemmh(n1, k, n2, C_ONE, U, ldU, V, ldV, data, n1);
  } else init0_GeM(n1, n2);
}


template<>
void mblock<scomp>::convLrM_toGeM()
{
  assert(isLrM());

  if (bl_rank>0) {
    scomp* const new_data = new scomp[n1*n2];
    assert(new_data!=NULL);

    blas::gemmh(n1, bl_rank, n2, C_ONE, data, n1, data+bl_rank*n1, n2,
                new_data, n1);
...
template<>
void mblock<scomp>::convLrM_toGeM(scomp* A, unsigned ldA) const
{
  assert(isLrM());
...
template<>
void mblock<scomp>::convGeM_toLrM(double eps)
{
  assert(isGeM());

  unsigned nmin = MIN(n1, n2), LWORK = 5*(n1+n2);
  int INFO;
  scomp *tmp = new scomp[(n1+nmin)*n2+LWORK];
  scomp *VT = tmp + n1*n2, *WORK = VT + nmin*n2;
  float *S = new float[nmin];
...
template<>
void mblock<scomp>::convGeM_toGeM(scomp* A, unsigned ldA) const
{
  assert(isGeM());
...
template<>
void mblock<scomp>::convHeM_toGeM(scomp* A, unsigned ldA) const
{
  assert(isHeM());
...
template<>
void mblock<scomp>::convSyM_toGeM(scomp* A, unsigned ldA) const
{
  assert(isSyM());
...
// appends lwr to lwr
template<>
void mblock<scomp>::append(unsigned k, scomp* U, unsigned ldU, scomp* V,
                           unsigned ldV)
{
  assert(isLrM());

  unsigned rank_new = bl_rank+k;
  scomp *tmp = new scomp[rank_new*(n1+n2)];
...
template<>
void mblock<scomp>::addtrll(unsigned k, scomp* U, unsigned ldU, scomp* V,
                            unsigned ldV, double delta, unsigned kgoal,
                            contLowLevel<scomp>* haarInfo,
			    scomp* X, unsigned ldX,
                            scomp* Y_, unsigned ldY_)
{
  assert(isLrM());
  ...
  if (k>0) {
    if (haarInfo==NULL) {
      ...
      scomp *tmp1=new scomp[size], *tmp2=tmp1+ksum*n1; // tmp2 = new V
      assert(tmp1!=NULL);
      ...
      int INFO;
      scomp* const WORK = tmp2 + n2*ksum; // LWORK
      scomp* const tau1 = WORK + LWORK;   // mmin
      scomp* const tau2 = tau1 + mmin;    // nmin
      ...
      scomp* const R = tau2 + nmin;       // mmin*nmin
      ...
      float* const S = new float[amin];
      scomp* const VT = R + mmin*nmin;    // amin*nmin
      ...
      if (kt>0) {
        ...
        for (j=0; j<kt; ++j) {
          ...
          for (; i<n1; ++i) data[i+j*n1] = C_ZERO;
        }

        scomp* dataV = data + kt*n1;

        // copy V to dataV
        for (j=0; j<kt; ++j) {
          unsigned i;
          for (i=0; i<nmin; ++i) dataV[i+j*n2] = conj(VT[i*amin+j]);
          for (; i<n2; ++i) dataV[i+j*n2] = C_ZERO;
        }
...
template<>
void mblock<scomp>::addtrll_rmnd(unsigned k, scomp* U, unsigned ldU, scomp* V,
                                 unsigned ldV, double delta, unsigned kgoal,
                                 unsigned& kR, scomp* &UR, scomp* &VR)
{
  assert(isLrM());
  kR = 0;
  UR = VR = NULL;
...
template<>
void mblock<scomp>::addGeM_toHeM(scomp* A, const unsigned ldA)
{
...
template<>
void mblock<scomp>::addGeM(scomp* A, unsigned ldA, double eps,
			   unsigned rankmax, contLowLevel<scomp>* haarInfo,
			   scomp* X, unsigned ldX, scomp* Y_, unsigned ldY_)
{
  if (isLrM()) {
    const unsigned min12 = MIN(n1, n2);
    const unsigned mtn = n1*n2;
    const unsigned lwork = 5*(n1+n2);
    scomp* const tmp = new scomp[mtn+min12*n2+lwork];
    assert(tmp!=NULL);
    ...
    if (haarInfo==NULL) {
      // compute SVD
      float* const S = new float[min12];
      scomp* const VT = tmp + mtn;           // min12*n2
      scomp* const work = VT + min12*n2;     // lwork
      ...
      int INFO = blas::gesvd(n1, n2, tmp, S, VT, min12, lwork, work);
      assert(INFO==0);
      scomp* const V = new scomp[min12*n2];
...
template<>
void mblock<scomp>::unify_cols_LrMLrM(double delta, unsigned kgoal,
				      const mblock<scomp>& mbl1,
				      const mblock<scomp>& mbl2,
				      contLowLevel<scomp>* haarInfo,
				      scomp* X, unsigned ldX,
				      scomp* Y_, unsigned ldY_)
{
  ...
  if (ksum>0) {
    ...
    scomp* const tmp=new scomp[(n1+1)*ksum+2*size+kU+LWORK+sizeV1+sizeV2];
    assert(tmp!=NULL);

    scomp* const R1 = tmp;                                    // n1*k1
    scomp* const R2 = tmp + sizeU1;                           // n1*k2
    scomp* const tau = R2 + sizeU2;                           // kU
    scomp* const tau1 = tau + kU;                             // k1
    scomp* const tau2 = tau1 + k1;                            // k2
    scomp* const M = tau2 + k2;                               // size
    float* const S = new float[kU];
    scomp* const VT = M + size;                               // size
    scomp* const WORK = VT + size;                            // LWORK
    scomp* const V1 = WORK + LWORK;                           // sizeV1
    scomp* const V2 = V1 + sizeV1;                            // sizeV2
    ...
    if (haarInfo==NULL) {
      ...
      if (kt>0) {
        data = new scomp[kt*(n1+n2)];
        assert(data!=NULL);

        INFO = blas::orgqr(n1, kU, tmp, tau, LWORK, WORK);
        assert(INFO==0);
        for (unsigned l=0; l<kt; ++l) {
          blas::scal(kU, S[l], M+l*kU);
        }
        blas::gemm(n1, kU, kt, C_ONE, tmp, n1, M, kU, data, n1);

        INFO = blas::orgqr(n2a, k1, V1, tau1, LWORK, WORK);
        assert(INFO==0);
        blas::gemmh(n2a, k1, kt, C_ONE, V1, n2a, VT, kU, data+kt*n1, n2);

        INFO = blas::orgqr(n2b, k2, V2, tau2, LWORK, WORK);
        assert(INFO==0);
        blas::gemmh(n2b, k2, kt, C_ONE, V2, n2b, VT+k1*kU, kU,
                    data+kt*n1+n2a, n2);
      }
    } else {
      scomp* const V = new scomp[size];
      for (unsigned i=0; i<kU; i++) {
      ...
      scomp* Xnew = new scomp[cols*ksum];
      scomp* Y_new = new scomp[cols*kU];
      blas::gemhm(n2a,k1,cols,C_ONE,V1,n2a,X,ldX,Xnew,ksum);
      blas::gemhm(n2b,k2,cols,C_ONE,V2,n2b,X+n2a,ldX,Xnew+k1,ksum);
      blas::gemhm(n1,kU,cols,C_ONE,tmp,n1,Y_,ldY_,Y_new,kU);
      createLowRankMatHouseholderNB(delta, kgoal, kU, kU, ksum, M, V, haarInfo,
                                    Xnew, ksum, Y_new, kU, bl_rank, datatemp);

      data = new scomp[bl_rank*(n1+n2)];
      assert(data!=NULL);
      blas::gemm(n1, kU, bl_rank, C_ONE, tmp, n1, datatemp, kU, data, n1);

      blas::gemm(n2a, k1, bl_rank, C_ONE, V1, n2a, &datatemp[bl_rank*kU], ksum,
                 data+bl_rank*n1, n2);

      blas::gemm(n2b, k2, bl_rank, C_ONE, V2, n2b, &datatemp[bl_rank*kU+k1],
...
// unify mblock<scomp>s (A, B) to U V^H with rank at most kgoal
template<>
void mblock<scomp>::unify_cols(double delta, unsigned kgoal,
                               const mblock<scomp>& mbl1,
                               const mblock<scomp>& mbl2,
                               contLowLevel<scomp>* haarInfo,
			       scomp* X, unsigned ldX,
                               scomp* Y_, unsigned ldY_)
{
  ...
  if (mbl1.isLrM() && mbl2.isLrM())
    ...
    if (mbl1.isLrM())
      blas::gemmh(n1, mbl1.bl_rank, n2a, C_ONE, mbl1.data, n1,
                  mbl1.data+mbl1.bl_rank*n1, n2a, tmp, n1);
    else
      blas::copy(n1*n2a, mbl1.data, tmp);

    if (mbl2.isLrM())
      blas::gemmh(n1, mbl2.bl_rank, n2b, C_ONE, mbl2.data, n1,
    ...
    if (haarInfo==NULL) {
      ...
      scomp* const V = new scomp[nmin*n2];
      for (unsigned i=0; i<n2; i++)
        for (unsigned j=0; j<nmin; j++)
...
// unify (U1 V1^H \\ U2 V2^H) to U V^H with rank at most kgoal
template<>
void mblock<scomp>::unify_rows_LrMLrM(double delta, unsigned kgoal,
				      const mblock<scomp>& mbl1,
				      const mblock<scomp>& mbl2,
				      contLowLevel<scomp>* haarInfo,
				      scomp* X, unsigned ldX,
				      scomp* Y_, unsigned ldY_)
{
  ...
  if (ksum>0) {
    ...
    scomp* const tmp=new scomp[(n2+1)*ksum+2*(size+kV)+LWORK+sizeU1+sizeU2];
    assert(tmp!=NULL);

    scomp* const R1 = tmp;                                    // n2*k1
    scomp* const R2 = tmp + sizeV1;                           // n2*k2
    scomp* const tau = R2 + sizeV2;                           // kV
    scomp* const tau1 = tau + kV;                             // k1
    scomp* const tau2 = tau1 + k1;                            // k2
    scomp* const M = tau2 + k2;                               // size
    scomp* const VT = M + size;                                 // size
    scomp* const WORK = VT + size;                            // LWORK
    scomp* const U1 = WORK + LWORK;                           // sizeU1
    scomp* const U2 = U1 + sizeU1;                            // sizeU2
    ...
    if (haarInfo==NULL) {
      ...
      if (kt>0) {
        ...
        assert(INFO==0);
        blas::gemm(n1a, k1, kt, C_ONE, U1, n1a, M, ksum, data, n1);

        INFO = blas::orgqr(n1b, k2, U2, tau2, LWORK, WORK);
        assert(INFO==0);
        blas::gemm(n1b, k2, kt, C_ONE, U2, n1b, M+k1, ksum, data+n1a, n1);

        INFO = blas::orgqr(n2, kV, tmp, tau, LWORK, WORK);
        assert(INFO==0);
        blas::gemmh(n2, kV, kt, C_ONE, tmp, n2, VT, kV, data+kt*n1, n2);
      }
    } else {
      scomp* const V = new scomp[size];
      ...
      scomp* datatemp;
      const unsigned cols = haarInfo->col;
      scomp* Xnew = new scomp[cols*kV];
      scomp* Y_new = new scomp[cols*ksum];
      blas::gemhm(n2,kV,cols,C_ONE,tmp,n2,X,ldX,Xnew,kV);
      blas::gemhm(n1a,k1,cols,C_ONE,U1,n1a,Y_,ldY_,Y_new,ksum);
      blas::gemhm(n1b,k2,cols,C_ONE,U2,n1b,Y_+n1a,ldY_,Y_new+k1,ksum);
      createLowRankMatHouseholderNB(delta, kgoal, ksum, kV, kV, M, V, haarInfo,
                                    Xnew, kV, Y_new, ksum, bl_rank, datatemp);

      data = new scomp[bl_rank*(n1+n2)];
      assert(data!=NULL);
      blas::gemm(n1a, k1, bl_rank, C_ONE, U1, n1a, datatemp, ksum, data, n1);

      blas::gemm(n1b, k2, bl_rank, C_ONE, U2, n1b, datatemp+k1, ksum,
                 data+n1a, n1);

      blas::gemm(n2, kV, bl_rank, C_ONE, tmp, n2, datatemp+bl_rank*ksum,
...
template<>
void mblock<scomp>::unify_rows(double delta, unsigned kgoal,
                               const mblock<scomp>& mbl1,
                               const mblock<scomp>& mbl2,
                               contLowLevel<scomp>* haarInfo,
			       scomp* X, unsigned ldX,
                               scomp* Y_, unsigned ldY_)
{
  ...
  if (mbl1.isLrM() && mbl2.isLrM())
    ...
    scomp *tmp = new scomp[n1*n2+nmin*(1+n2)+LWORK];
    assert(tmp!=NULL);

    // convert (A \\ B) to dense matrix tmp
    if (mbl1.isLrM())
      blas::gemmh(n1a, mbl1.bl_rank, n2, C_ONE, mbl1.data, n1a,
    ...
    if (mbl2.isLrM())
      blas::gemmh(n1b, mbl2.bl_rank, n2, C_ONE, mbl2.data, n1b,
    ...
    if (haarInfo==NULL) {
      ...
      for (unsigned i=0; i<nmin; i++)
        blas::scal(n1,S[i],&tmp[i*n1]);
      createLowRankMatHouseholderNB(delta, kgoal, n1, nmin, n2, tmp, V,
				    haarInfo, X, ldX, Y_, ldY_, bl_rank, data);
      delete [] V;
    }
...
template<>
int mblock<scomp>::decomp_LU(mblock<scomp>* L, mblock<scomp> *U)
// destroys data
{
  assert(n1==n2);
  ...
  scomp *pA = data, *pL = L->data, *pU = U->data;
...
template<>
void mblock<scomp>::mltaLtMVec(scomp d, scomp* x, scomp* y) const
{
  assert(isGeM() && isLtM() && n1==n2);
  unsigned *ip = new unsigned[n1];
  scomp* z = new scomp[n1];
  blas::setzero(n1, z);
...
template<>
void mblock<scomp>::mltaUtMVec(scomp d, scomp* x, scomp* y) const
{
...
template<>
void mblock<scomp>::mltaLtMhVec(scomp d, scomp* x, scomp* y) const
{
  assert(isGeM() && isLtM() && n1==n2);
  scomp *p = data, *z = new scomp[n1];
...
template<>
void mblock<scomp>::mltaUtMhVec(scomp d, scomp* x, scomp* y) const
{
  assert(isGeM() && isUtM());
  scomp *p = data;
...
template<>
void mblock<scomp>::ltr_solve(unsigned m, scomp* B, unsigned ldB) const
{
  assert(isGeM() && isLtM() && n1==n2);
  scomp *p = data, *Z = new scomp[n1*m];
...
template<>
void mblock<scomp>::ltrh_solve(unsigned m, scomp* B, unsigned ldB) const
{
  assert(isGeM() && isLtM() && n1==n2);
  scomp *p = data, *Z = new scomp[n1*m];
  unsigned j, *ip = new unsigned[n1];
...
template<>
void mblock<scomp>::utr_solve_left(unsigned m, scomp* B, unsigned ldB,
                                   scomp* X, unsigned ldX) const
{
  assert(isGeM() && isUtM());
  scomp *p = data;
...
template<>
void mblock<scomp>::addLrM_toHeM(unsigned mult, unsigned k, scomp* U,
				 unsigned ldU, scomp* V, unsigned ldV)
{
...
// add lwr to herm. dense
template<>
void mblock<scomp>::addLrM_toUtM(unsigned mult, unsigned k, scomp* U,
				 unsigned ldU, scomp* V, unsigned ldV)
{
  assert(isGeM() && isUtM());
  for (unsigned l=0; l<k; ++l)
...
// add lwr to mblock<scomp> and truncate to min(eps, kgoal)
template<>
void mblock<scomp>::addLrM(unsigned k, scomp* U, unsigned ldU, scomp* V,
...
// add lwr to mblock<scomp> and
template<>
void mblock<scomp>::addLrM_Exact(unsigned k, scomp* U, unsigned ldU, scomp* V,
				 unsigned ldV)
{
  if (isLrM()) { // cannot be symmetric since it is lwr
...
// add lwr to mblock<scomp> and truncate to min(eps, kgoal)
template<>
void mblock<scomp>::addLrM_rmnd(unsigned k, scomp* U, unsigned ldU, scomp* V,
				unsigned ldV, double eps, unsigned kgoal,
				unsigned& kR, scomp* &UR, scomp* &VR)
{
  if (isLrM()) {
